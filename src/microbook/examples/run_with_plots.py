...
def seed_book(sim: MarketSimulator):
    sim.schedule(0, EventType.SUBMIT, order=Order("seed_s1", Side.SELL, 101.0, 80, ts=0))
    sim.schedule(0, EventType.SUBMIT, order=Order("seed_b1", Side.BUY,  99.0, 80, ts=0))

def main():
    mm = MarketMaker(tick_interval=10, size=5, half_spread_ticks=1)
    twap = TWAPExecutor(Side.BUY, total_qty=60, start=50, end=300, tick_interval=20, name="twap_buy")

    sim = MarketSimulator(strategies=[mm, twap])
    seed_book(sim)

    flow = PoissonOrderFlow(FlowConfig(seed=7, intensity_per_100=45.0, p_market=0.12))
    ...
    # Strategy PnL curves (mark-to-market at end only for now)
